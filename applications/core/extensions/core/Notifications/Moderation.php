...
{	
	 ...
	 * @return	void
	 */
	public static function disableExtra( \IPS\Member $member = NULL, $method )
	{
		// Do nothing
	}
	...
	public function parse_unapproved_content( $notification, $htmlEscape=TRUE )
	{
		...
		return array(
			'title'		=> \IPS\Member::loggedIn()->language()->addToStack( 'notification__unapproved_content', FALSE, array( ( $htmlEscape ? 'sprintf' : 'htmlsprintf' ) => array( $item->author()->name, mb_strtolower( $item->indefiniteArticle() ), $title ) ) ),
			'url'		=> $item->url(),
