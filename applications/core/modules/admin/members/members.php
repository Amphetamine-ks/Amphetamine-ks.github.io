...
class _members extends \IPS\Helpers\CoverPhoto\Controller
{
	...
	protected function manage()
	{
		...
		/* Ranks */
		$ranks = \IPS\Db::i()->select( '*, w.word_custom as title', 'core_member_ranks', NULL, 'posts ASC' )->join( array( 'core_sys_lang_words', 'w' ), "w.word_key=CONCAT( 'core_member_rank_', core_member_ranks.id ) AND w.lang_id=" . \IPS\Member::loggedIn()->language()->id );
		
		$rankOptions = array( '' => 'any' );
		foreach( $ranks as $rank )
		{
			$rankOptions[ $rank['id'] ] = $rank['title'];
		}
		...
		$table->advancedSearch = array(
			...
			'member_rank_id'	=> array( \IPS\Helpers\Table\SEARCH_SELECT, array( 'options' => $rankOptions ), function( $val ) use ( $ranks )
			{
				$minPost = 0;
				$maxPost = 0;
				$minSet = FALSE;
				foreach( $ranks as $rank )
				{
					...
					if ( $minSet and $rank['posts'] > $minPost )
					{
						$maxPost = $rank['posts'];
						break;
					}
				...
				if ( $minPost and $maxPost )
				{
					return array( '( member_posts BETWEEN ? AND ? )', $minPost, $maxPost - 1 );
				}
				elseif ( $minPost )
				{
					return array( '( member_posts >= ? )', $minPost );
				}
			} ),
		...
		$table->parsers = array(
			...
			'member_rank_id' => function( $val, $row )
			{
				$member = \IPS\Member::constructFromData( $row );
				$member->member_title = NULL;
				return $member->rank['title'];
			},		
		);
		...
		if( \IPS\Request::i()->advanced_search_submitted OR \IPS\Request::i()->quicksearch )
		{
			...
			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'members', 'member_delete' ) OR \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'members', 'member_edit' ) )
			{
				...
				{
					/* Add profile fields */
					if ( mb_substr( $k, 0, 14 ) === 'members_field_' and $v and ( \is_array( $v ) and \array_search( '__EMPTY', $v ) === FALSE ) )
					{
						$query[ $k ] = $v;
			...
			array(
				...
				'import_do_import'	=> function( $wizardData ) use ( $maxMemberId )
				{
					return (string) new \IPS\Helpers\MultipleRedirect( \IPS\Http\Url::internal('app=core&module=members&controller=members&do=import'),	function( $mrData ) use ( $wizardData, $maxMemberId )
					{
						...
						{
							\IPS\Email::buildFromTemplate( 'core', 'admin_reg', array( $member, $member->members_bitoptions['password_reset_forced'], md5( \IPS\SUITE_UNIQUE_KEY . $member->email . $member->real_name ) ) )->send( $member );
						}
