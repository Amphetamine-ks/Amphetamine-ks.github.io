...
class _notifications extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		$methodTable = new \IPS\Helpers\Table\Custom( array( 'inline' => array( 'method' => \IPS\Member::loggedIn()->language()->addToStack('member_notifications_inline') ) ), \IPS\Http\Url::internal( 'app=core&module=membersettings&controller=notifications', 'admin' ) );
		$methodTable->langPrefix = 'notificationsettings_';
		$methodTable->rowButtons = function( $row, $k ) {
			return array(
				'edit'	=> array(
					'title'	=> 'edit',
					'link'	=> \IPS\Http\Url::internal( 'app=core&module=membersettings&controller=notifications&do=method', 'admin' )->setQueryString( 'id', $k ),
					'icon'	=> 'pencil'
				)
			);
		};
		...
		\IPS\Output::i()->output = $typeTable . $methodTable;
	}
	...
	protected function edit()
	{
		...
		foreach ( \IPS\Notification::availableOptions( NULL, $extension ) as $key => $option )
		{
			if ( $option['type'] === 'standard' )
			{
				...
				if ( !\in_array( 'inline', $option['disabled'] ) or ( \IPS\Settings::i()->mobile_app_id and !\in_array( 'push', $option['disabled'] ) ) or !\in_array( 'email', $option['disabled'] ) )
				{
	...
	protected function method()
	{
		...
		$form->add( new \IPS\Helpers\Form\Radio( 'notification_prefs_sound', \IPS\Settings::i()->notification_prefs_sound ? 'default' : 'optional', FALSE, array( 'options' => array(
			'default'			=> 'admin_notification_pref_default',
			'optional'			=> 'admin_notification_pref_optional',
		) ), NULL, NULL, NULL, 'notification_prefs_sound' ) );

		if ( $values = $form->values() )
		{
			$values['notification_prefs_sound'] = ( $values['notification_prefs_sound'] === 'default' );
	...
	protected function updateMemberFollowPrefs()
	{
		...
		/* Sounds is special */
		\IPS\Db::i()->update( 'core_members', 'members_bitoptions2 = members_bitoptions2 ' . ( !\IPS\Settings::i()->notification_prefs_sound ? '|' : '&~' ) . \IPS\Member::$bitOptions['members_bitoptions']['members_bitoptions2']['disable_notification_sounds'] );
