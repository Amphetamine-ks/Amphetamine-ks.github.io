...
class _upgrade extends \IPS\Dispatcher\Controller
{
	...
	public function _selectVersion( $data )
	{
		...
		{			
			...
			/* Get details */
			$data = array( 'version' => $values['version'], 'oldVersion' => \intval( \IPS\Application::getAvailableVersion('core') ), 'changes' => array() );
	...
	public function _upgradeLanguages( $data, $stepData )
	{
		return $this->_appLoop( 'lang', $data, $stepData, function( $app, $data, $stepData )
		{
			...
			$inserts = array();
			$xml = new \IPS\Xml\XMLReader;
			$xml->open( \IPS\ROOT_PATH . "/applications/{$app}/data/lang.xml" );
			$xml->read();
			$xml->read();
			$xml->read();
