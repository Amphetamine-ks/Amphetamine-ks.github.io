...
class _marketplace extends \IPS\Dispatcher\Controller
{
	 ...
	 * @return	void
	 */
	final protected function install(): void
	{
	 ...
	 * @return	void
	 */
	final protected function viewCategory(): void
	{
	 ...
	 * @return	void
	 */
	final protected function viewFile(): void
	{
		$file = $this->_apiWrapper( "marketplace/files/" . (int) \IPS\Request::i()->id, [ 'termFields' => 1 ], TRUE );
	 ...
	 * @return	void
	 */
	final protected function buyFile(): void
	{
		\IPS\Session::i()->csrfCheck();
		...
		if ( $this->_token() )
		{
			$response = $this->_apiWrapper( "downloads/files/" . (int) \IPS\Request::i()->id . "/buy", NULL, TRUE, array(
				'returnUri'	=> 'app=core&module=system&controller=ajax&do=dismissWindow'
			) );
	 ...
	 * @return	void
	 */
	final protected function fileChangelog(): void
	{
	 ...
	 * @return	void
	 */
	final protected function signout(): void
	{
		\IPS\Session::i()->csrfCheck();
	 ...
	 * @return	void
	 */
	final protected function apiSearch(): void
	{
		$queryString = [
		...
		if ( \IPS\Request::i()->single )
		{
			...
			{
				\IPS\Output::i()->sendOutput( '', 200, 'text/plain' );
			}
		}
		else
		{
			...
			if ( $ids )
			{
				...
				{
					\IPS\Output::i()->sendOutput( \IPS\Member::loggedIn()->language()->addToStack( 'no_results' ), 200, 'text/plain' );
				}
				else
				{
					\IPS\Output::i()->sendOutput( \IPS\Theme::i()->getTemplate('marketplace')->fileCardList( $results['results'] ) );
				}
			}
			else
			{
	 ...
	 * @return	void
	 */
	final protected function apiLookup(): void
	{
		try
		{
			\IPS\Output::i()->sendOutput( \IPS\Theme::i()->getTemplate('marketplace')->fileCardMini( $this->_api( "downloads/files/" . (int) \IPS\Request::i()->id ) ) );
		}
		catch ( \Exception $e )
		{
			\IPS\Output::i()->sendOutput( '', 200, 'text/plain' );
	 ...
	 */
	final protected function _api( string $endpoint, array $queryString = NULL, bool $authenticate = FALSE, array $data = NULL, int $timeout = \IPS\DEFAULT_REQUEST_TIMEOUT ): array
	{
		$token = $authenticate ? ( $this->_token() ?? \IPS\MARKETPLACE_GUEST_TOKEN ) : \IPS\MARKETPLACE_GUEST_TOKEN;
		$url = \IPS\Http\Url::external( \IPS\MARKETPLACE_URL . '/api/' . $endpoint )->setQueryString( 'ips_version', \IPS\Application::load('core')->long_version );
		if ( $queryString )
		{
	...
	final protected function _customHeader( string $title, array $file=NULL, array $categoryIds=NULL ): string
	{
		...
		else
		{
			try
			{
				...
				$memberData = $response->decodeJson();
				$authenticatedMember = $memberData['name'];
			}
			catch ( \Exception $e )
			{
	 ...
	 */
	final protected function _installApplication( array $file, string $temporaryFile ): void
	{
		/* Test the phar */
	 ...
	 */
	final protected function _installLanguage( array $file, string $temporaryFile ): void
	{
		/* Already installed? */
		$xml = new \IPS\Xml\XMLReader;
		$xml->open( $temporaryFile );
		if ( !@$xml->read() )
		{
			throw new \UnexpectedValueException;
		}
	 ...
	 */
	final protected function _installPlugin( array $file, string $temporaryFile ): void
	{
		/* Already installed? */
		$xml = new \IPS\Xml\XMLReader;
		$xml->open( $temporaryFile );
	 ...
	 */
	final protected function _installTheme( array $file, string $temporaryFile ): void
	{
		$xml = new \IPS\Xml\XMLReader;
		$xml->open( $temporaryFile );
		if ( !@$xml->read() )
	 ...
	 * @return	void
	 */
	final protected function _setDatastoreCache( string $name, array $content, int $ttl ): void
	{
		$ttlSeconds = $ttl * 60;
		$variableName = 'marketplace_' . $name . '_' . md5( $this->_token() ?? \IPS\MARKETPLACE_GUEST_TOKEN );
	 ...
	 * @return 	array
	 */
	final public function _updateCheck( array $files ):? array
	{
		return $this->_api("marketplace/files/versions", [ 'files' => $files ] );
	}
