...
class _onboard extends \IPS\Dispatcher\Controller
{
	...
	{
		$steps = [];
		if ( array_filter( array_keys( \IPS\Application::applications() ), function( $k ) { return !\in_array( $k, \IPS\Application::$ipsApps ); } ) )
		{
	 ...
	 * @return	mixed
	 */
	public function _applications( $data )
	{
		...
		foreach ( \IPS\Application::applications() as $application )
		{
			if ( !\in_array( $application->directory, \IPS\Application::$ipsApps ) )
			{
	 ...
	 * @return	mixed
	 */
	public function _languages( $data )
	{
