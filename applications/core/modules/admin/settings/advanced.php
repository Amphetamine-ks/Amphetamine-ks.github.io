...
class _advanced extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageDatastore()
	{
		...
		/* Remove deprecated options if they're not in use @todo 4.6 */
		$deprecatedCacheMethods = array( 'Apc', 'Memcache', 'Wincache', 'Xcache' );
		foreach( $deprecatedCacheMethods as $cache )
		{
			if( $cache !== \IPS\CACHE_METHOD )
			{
				unset( $options[ $cache ] );
			}
		}
		...
		foreach ( $options as $k => $v )
		{
			...
			{				
				...
				/* Deprecated Warnings */
				if( \in_array( $k, $deprecatedCacheMethods ) )
				{
					\IPS\Member::loggedIn()->language()->words["datastore_method_{$k}_desc"] = \IPS\Member::loggedIn()->language()->addToStack( 'datastore_method_deprecated', FALSE, array( 'sprintf' => array( $k ) ) );
				}
			}
		}
