	...
	public function run()
	{
		...
		foreach ( $request->articles( $this->id ) as $guid => $article )
		{
			if ( !\in_array( $guid, $previouslyImportedGuids ) )
			{
				...
				$imageHtml = '';
				if ( isset( $article['enclosure'] ) and isset( $article['enclosure']['url'] ) and isset( $article['enclosure']['length'] ) and isset( $article['enclosure']['type'] ) )
				{
					/* Limit imports to around 2mb otherwise timeouts and memory issues can occur when processing with GD */
					if ( mb_substr( $article['enclosure']['type'], 0, 6 ) == 'image/' and ( $article['enclosure']['length'] <= 1572864 or \IPS\Settings::i()->image_suite == 'imagemagick' ) )
					{
						try
						{
				...
				$content = $imageHtml . \IPS\Text\Parser::parseStatic( $article['content'] . $readMoreLink, TRUE, NULL, $member, $this->_extension->fileStorage, TRUE, !(bool) $member->group['g_dohtml'] );
