...
class _view extends \IPS\Dispatcher\Controller
{
	...
	protected function _viewMonth( $jump, $featured=array() )
	{
		...
		try
		{
			...
			/* Get birthdays */
			$birthdays	= ( $this->_calendar === NULL OR \count( $this->roots ) == 1 ) ? $date->getBirthdays( FALSE, TRUE ) : array();
	...
	protected function _viewDay( $jump, $featured=array() )
	{
		...
		$birthdayCount	= ( $this->_calendar === NULL OR \count( $this->roots ) == 1 ) ? $date->getBirthdays( TRUE, TRUE ) : array();
		$birthdays		= ( $this->_calendar === NULL OR \count( $this->roots ) == 1 ) ? $date->getBirthdays( TRUE, FALSE ) : array();
