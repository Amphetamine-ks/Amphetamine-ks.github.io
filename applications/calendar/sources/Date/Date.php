...
class _Date extends \IPS\DateTime
{
	...
	public function mysqlDatetime( $includeTime=TRUE )
	{
		if( $includeTime )
		{
			return $this->dateInformation['year'] . '-' . str_pad( $this->dateInformation['mon'], 2, 0, STR_PAD_LEFT ) . '-' . str_pad( $this->dateInformation['mday'], 2, 0, STR_PAD_LEFT ) . ' ' . $this->dateInformation['hours'] . ':' . $this->dateInformation['minutes'] . ':' . $this->dateInformation['seconds'];
		}
		else
		{
