...
class _Woltlab extends \IPS\convert\Software
{
	...
	public static function canConvert()
	{
		return array(
			...
			'convertRanks'					=> array(
				'table'							=> 'wcf' . static::$installId . '_user_rank',
				'where'							=> array( 'requiredPoints>0' )
			),
	...
	public function convertMembers()
	{
		...
		foreach ( $this->fetch( 'wcf' . static::$installId . '_user', 'userID' ) AS $row )
		{
			...
			$info = array(
				...
				'member_title'			=> \in_array( $row['userTitle'], $userTitles ) OR empty( $row['userTitle'] ) ? NULL : $row['userTitle'],
	...
	{
		$libraryClass = $this->getLibrary();
		$libraryClass::setKey( 'rankID' );

		foreach( $this->fetch( 'wcf' . static::$installId . '_user_rank', 'rankID', array( 'requiredPoints>0' ) ) AS $row )
		{
			$libraryClass->convertRank( array(
				'id'			=> $row['rankID'],
				'title'			=> $row['rankTitle'],
				'posts'			=> $row['requiredPoints']
			) );

			$libraryClass->setLastKeyValue( $row['rankID'] );
		}
	}
