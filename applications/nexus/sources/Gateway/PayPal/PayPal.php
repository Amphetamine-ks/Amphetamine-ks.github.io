...
class _PayPal extends \IPS\nexus\Gateway
{
	...
	protected function _billingAgreementAuth( \IPS\nexus\Transaction $transaction, \IPS\nexus\Fraud\MaxMind\Request $maxMind = NULL, \IPS\nexus\Purchase\RenewalTerm $term, $items )
	{
		...
		/* Create a plan */
		$renewalAmountIncludingTax = $term->cost->amount->multiply( new \IPS\Math\Number( number_format( $term->tax ? ( (string) ( 1 + $term->tax->rate( $transaction->invoice->billaddress ) ) ) : '1', \IPS\nexus\Money::numberOfDecimalsForCurrency( $term->cost->currency ), '.', '' ) ) );
		$billingCycle = array(
	...
	protected static function _canProcessRecurringsAsBillingAgreement( array $recurrings )
	{
		if ( \count( $recurrings ) == 1 )
		{
			...
			if ( $recurrance['dueDate']->getTimestamp() === \IPS\DateTime::create()->add( $recurrance['term']->interval )->getTimestamp() )
			{
				return TRUE;
			}
		}
