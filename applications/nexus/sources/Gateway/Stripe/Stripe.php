...
class _Stripe extends \IPS\nexus\Gateway
{
	...
	public function testSettings( $settings )
	{
		try
		{
			...
			if ( !$webhookId )
			{
				throw new \DomainException( \IPS\Member::loggedIn()->language()->addToStack( 'stripe_webhook_invalid', FALSE, array( 'sprintf' => array( \IPS\Member::loggedIn()->language()->formatList( $this->webhookEvents ) ) ) ) );
			}
