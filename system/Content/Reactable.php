		...
		$table->parsers = array(
			'rep_date' => function( $date, $row )
			{
				if ( isset( \IPS\Request::i()->item ) and \IPS\Request::i()->item )
				{
					...
					{
						$class = $row['rep_class'];
						$comment = $class::load( $row['type_id'] );

						return \IPS\Member::loggedIn()->language()->addToStack( '_defart_from_date', NULL, array( 'htmlsprintf' => array( $comment->url(), $comment::$title, \IPS\DateTime::ts( $date )->html() ) ) );
					}
					catch( \Exception $e )
					{
