...
abstract class _Item extends \IPS\Content
{
	...
	public function delete()
	{
		...
		if ( isset( static::$commentClass ) )
		{
			...
			\IPS\Db::i()->delete( 'core_solved_index', array( "comment_class=? AND " . \IPS\Db::i()->in( 'comment_id', $commentIds ), $commentClass ) );
		...
		if ( isset( static::$reviewClass ) )
		{
			...
			\IPS\Db::i()->delete( 'core_solved_index', array( "comment_class=? AND " . \IPS\Db::i()->in( 'comment_id', $reviewIds ), $reviewClass ) );
	...
	public static function getItemsWithPermission( $where=array(), $order=NULL, $limit=10, $permissionKey='read', $includeHiddenItems=\IPS\Content\Hideable::FILTER_AUTOMATIC, $queryFlags=0, \IPS\Member $member=NULL, $joinContainer=FALSE, $joinComments=FALSE, $joinReviews=FALSE, $countOnly=FALSE, $joins=NULL, $skipPermission=FALSE, $joinTags=TRUE, $joinAuthor=TRUE, $joinLastCommenter=TRUE, $showMovedLinks=FALSE )
	{
		...
		elseif ( \in_array( 'IPS\Content\Hideable', class_implements( \get_called_class() ) ) and $includeHiddenItems !== \IPS\Content\Hideable::FILTER_SHOW_HIDDEN )
		{
			...
			if ( isset( static::$databaseColumnMap['approved'] ) )
			{
				...
				{
					/* Only fetching a count, for a single container, no future publishing, and there is only one where clause (the container limitation) */
					if( $countOnly === TRUE AND $skipPermission instanceof \IPS\Node\Model AND !\in_array( 'IPS\Content\FuturePublishing', class_implements( \get_called_class() ) ) AND \count( $where ) === 1 )
					{
						$countShortcut = TRUE;
		...
		else
		{
			if ( ( $queryFlags & static::SELECT_IDS_FIRST or \IPS\CIC ) or $groupBy )
			{
				...
				if ( $pass === true )
				{
					...
					if( $order == '_rand' )
					{
						$subSelectClause	.= static::_getRandomizationSql( static::$databaseTable . '.' . static::$databasePrefix . static::$databaseColumnMap['date'] );
					}
			...
			if( $order == '_rand' )
			{
				$selectClause	.= static::_getRandomizationSql( static::$databaseTable . '.' . static::$databasePrefix . static::$databaseColumnMap['date'] );
			}
	 ...
	 *
	 * @param	string		$column		Column to use for randomization
	 * @return	string
	 */
	protected static function _getRandomizationSql( $column )
	{
		return ', CONCAT( SUBSTR(' . $column . ', ' . '10, 10 ), SUBSTR(' . $column . ', ' . '9, 10 ), SUBSTR( ' . $column . ', ' . rand( 1, 9 ) . ', 10 ), SUBSTR( ' . $column . ', ' . rand( 4, 7 ) . ', 9 ) ) as _rand';
	}
	...
	protected function _comments( $class, $limit, $offset=NULL, $order='date DESC', $member=NULL, $includeHidden=NULL, $cutoff=NULL, $canViewWarn=NULL, $extraWhereClause=NULL, $includeDeleted=FALSE )
	{
		...
		elseif ( $this->mapped('num_comments') >= 500 )
		{
			...
			if( method_exists( $this, 'forceIndexForPaginatedIds' ) and $this->forceIndexForPaginatedIds() )
			{
		...
		{
			if ( $this->containerAllowsMemberSolvable() OR $this->containerAllowsSolvable() )
			{
				$memberIds = array();
				$solvedCounts = array();
				$commentClass = static::$commentClass;
	 ...
	 * @return	array
	 */
	public function generateCommentMetaData( array $comments ) : array
	{
			...
			if ( $this->showCommentMeta('moderation') )
			{
				...
				foreach ( $moderationItems as $id => $data )
				{
					...
					if ( ( $modActionDate > $commentDate and $modActionDate < $nextCommentDate ) or ( $modActionDate > $commentDate and !$next )  )
					{
						...
						$blurb = NULL;
						$langKey = 'comment_meta_moderation_' . $data['lang_key'];
					...
					foreach( $otherActions as $groupIdx => $actions )
					{
						if( \count( $actions ) === 1 )
						{
							...
							$comment->_data['metaData']['comment']['moderation'][$actions[0]['action']] = array(
								'row' => $actions[0]['row'],
								'blurb' => \IPS\Member::loggedIn()->language()->addToStack( $actions[0]['lang_key'], FALSE, array('htmlsprintf' => array($this->definiteArticle(), \IPS\Member::load( $lastActionMember )->link())) ),
						...
						else
						{
							...
							$comment->_data['metaData']['comment']['moderation'][$groupIdx] = array(
								...
								'blurb' => \IPS\Member::loggedIn()->language()->addToStack( 'comment_meta_moderation_modlog__action_group', FALSE, array('htmlsprintf' => array($this->definiteArticle(), \IPS\Member::load( $lastActionMember )->link(), \IPS\Member::loggedIn()->language()->formatList( $actionPhrases ) ) ) ),
								'action' => $actions[0]['lang_key']
							);
						}
					}
	 ...
	 * @return	void
	 */
	public function toggleItemModeration( $action, ?\IPS\Member $member = NULL ): void
	{
