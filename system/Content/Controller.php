...
class _Controller extends \IPS\Helpers\CoverPhoto\Controller
{
	...
	public function solve()
	{
		...
		{
			$notification = new \IPS\Notification( \IPS\Application::load( 'core' ), 'best_answer', $item, array( $comment ), array( $comment->author()->member_id ) );
	...
	public function toggleItemModeration()
	{
		...
		try
		{
			...
			\IPS\Session::i()->modLog( 'modlog__item_moderation_toggled', [$action => FALSE], $item );
			
			\IPS\Output::i()->redirect( $item->url(), ( $item->itemModerationEnabled() ) ? 'enabled' : 'disabled' );
		}
		catch( \OutOfRangeException $e )
		{
		...
		else
		{
			if ( $comment->canDelete() )
			{
				...
				if ( \IPS\Settings::i()->dellog_retention_period AND ( $item instanceof \IPS\Content\Hideable ) AND !isset( \IPS\Request::i()->immediately ) )
				{
					...
					if ( \IPS\IPS::classUsesTrait( $item, 'IPS\Content\Solvable' ) and isset( $item::$databaseColumnMap['solved_comment_id'] ) )
					{
						/* Reset item */
						$idColumn = $commentClass::$databaseColumnId;
						$itemIdColumn = $item::$databaseColumnId;
						if ( $item->mapped('solved_post_id') and $item->mapped('solved_post_id') == $comment->$idColumn )
						{
							\IPS\Db::i()->update( $item::$databaseTable, array( $item::$databaseColumnMap['solved_comment_id'] => 0 ), array( $itemIdColumn . '=?', $item->$itemIdColumn ) );
						}
					}
				}
				else
