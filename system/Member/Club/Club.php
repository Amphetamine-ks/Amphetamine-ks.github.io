...
class _Club extends \IPS\Patterns\ActiveRecord implements \IPS\Content\Embeddable
{
	 ...
	 */
	public static function clubs( \IPS\Member $member = NULL, $limit, $sortOption, $mineOnly=FALSE, $filters=array(), $extraWhere=NULL, $countOnly=FALSE )
	{
		$where = array();
		$joins = array();
	...
	public function form( $acp=FALSE, $new=FALSE, $availableTypes=NULL )
	{
		...
		foreach ( \IPS\Member\Club\CustomField::roots() as $field )
		{
			if ( $field->type === 'Editor' )
			{
				...
				{
					$field::$editorOptions = array_merge( $field::$editorOptions, array( 'attachIds' => array( $this->id, $field->id, NULL ) ) );
				}
			}
			...
			if ( $field->type === 'Editor' )
			{
				if ( $field->allow_attachments AND $new )
				{
					$field::$editorOptions = array_merge( $field::$editorOptions, array( 'attachIds' => array( NULL ) ) );
				}
			}
			$form->add( $helper );
		}
	 ...
	 */
	public function acknowledgeRules( ?\IPS\Member $member = NULL ): void
	{
		$member = $member ?: \IPS\Member::loggedIn();
