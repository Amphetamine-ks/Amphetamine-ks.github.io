...
class _Number implements \JsonSerializable
{
	...
	public function divide( $number, $precision = 3, $round = \IPS\Math\Number::ROUND_TRUNCATE )
	{
		...
		{
			$result = str_replace( '.', '', bcdiv( (string) $this, (string) $number, $precision ) ); // It will automatically return with correct $precision - e.g. "1.000" rather than "1", so we can just strip the decimal to get the absolute value we need
		}
