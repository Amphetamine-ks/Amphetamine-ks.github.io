...
class _Application extends \IPS\Node\Model
{
	 ...
	 * @deprecated - see \IPS\IPS::$ipsApps
	 */
...
EOF;
	...
	public function installLanguages( $offset=null, $limit=null )
	{
		...
		{			
			...
			$xml->open( $this->getApplicationPath() . "/data/lang.xml" );
			$xml->read();
	...
	public function installEmailTemplates()
	{
		if( file_exists( $this->getApplicationPath() . "/data/emails.xml" ) )
		{
			...
			/* Open XML file */
			$xml = new \IPS\Xml\XMLReader;
			$xml->open( $this->getApplicationPath() . "/data/emails.xml" );
			$xml->read();
	...
	public function installTemplates( $update=FALSE, $offset=null, $limit=null )
	{
		...
		if( file_exists( $this->getApplicationPath() . "/data/theme.xml" ) )
		{
			...
			/* Open XML file */
			$xml = new \IPS\Xml\XMLReader;
			$xml->open( $this->getApplicationPath() . "/data/theme.xml" );
			$xml->read();
	...
	public function buildNewFeatures()
	{
		...
		\IPS\Db::i()->update( 'core_members_features', array( 'feature_version_added' => $this->long_version ), array( "feature_version_added=? AND feature_app=?", 0, $this->directory ) );
		...
		if ( file_exists( \IPS\ROOT_PATH . "/applications/{$appKey}/setup/upg_{$versionId}/whatsnew.json" ) )
		{
			if ( $data = @json_decode( \file_get_contents( \IPS\Application::load( $appKey )->getApplicationPath() . "/setup/upg_{$versionId}/whatsnew.json" ), TRUE ) )
			{
				...
				foreach( $data AS $uid => $row )
				{
					...
					unset( $row['_image_data'] );
					$insert[] = $row;
				}
				
				\IPS\Db::i()->insert( 'core_members_features', $insert );
			}
		}
	}
