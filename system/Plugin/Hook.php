			...
			foreach ( $reflection->getMethods() as $method )
			{
				if ( !$method->isInternal() )
				{
					...
					foreach ( $method->getParameters() as $parameter )
					{
						try
						{
							$parameters[] = ( $parameter->getClass() ? ( ( ( $parameter->allowsNull() ) ? '?' : '' ). '\\' . $parameter->getClass()->getName() . ' ' ) : '' ) . ( $parameter->isPassedByReference() ? '&' : '' ) . '$' . $parameter->name . ( $parameter->isOptional() ? ( '=' . var_export( $parameter->getDefaultValue(), TRUE ) ) : '' );
						}
